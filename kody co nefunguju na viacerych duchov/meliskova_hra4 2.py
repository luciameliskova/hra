...
# toto nepise ziadny error ale inak to vobec nic neorbi
...
# dlasi pokus na urobenie duchov

def vytvor_zloduchov(platno, sirka, vyska, mapa, zloduch, cell_size):
    zloduchovia = []
    for y in range(0, vyska):
        for x in range(0, sirka):
            if mapa[y][x] == 'z':
                obrazok_zloducha = platno.create_image(x * cell_size + cell_size / 2, \
                                                       y * cell_size + cell_size / 2, \
                                                       image=zloduch)
                zloduch.append(obrazok_zloducha)
...
#?zloduch = tkinter.PhotoImage(file = '/Users/lucka/Desktop/duc.gif')
#?zloduchh = platno.create_image(image = zloduch)

#zloduch = platno.create_image(vytvor_zloduchov(platno, sirka, vyska, mapa, zloduch, cell_size)
